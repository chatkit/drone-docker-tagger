...
func main() {
	...
	app := cli.NewApp()
	app.Name = "docker tagger plugin"
	app.Usage = "docker tagger plugin"
	app.Action = run
	...
	app.Flags = []cli.Flag{
		...
		cli.StringFlag{
			Name:   "source_image",
			Usage:  "source image to create tags from",
...
func run(c *cli.Context) error {
	...
	plugin := Plugin{
		...
		Build: Build{
			...
			Pull:        c.BoolT("pull_image"),
			SourceImage: c.String("source_image"),
		},
		Daemon: Daemon{
			Registry:      c.String("docker.registry"),
